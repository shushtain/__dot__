#let ch(n) = n * measure([0]).width
#let rem(n) = n * context text.size

#let _calc_margin(old, height, base) = {
  let frame = height - old
  let rest = calc.rem(frame / 1pt, base / 1pt) * 1pt
  old + rest
}
...
#let setup(
  lang: "en",
  font-size: 12pt,
  margin: (left: 42pt, right: 42pt, top: 36pt, bottom: 36pt),
  header: none,
  footer: none,
  guides: false,
  link_underline: true,
  ...
  let margin = margin
  margin.bottom = _calc_margin(margin.bottom, page.height, font-size / 2)

  let header = header
  if header != none {
    margin.top += font-size
    let base = text.size / 2
    set text(size: base / 0.6, top-edge: base)
    header = stack(
      spacing: base,
      text(size: base / 0.6, top-edge: base, header),
      line(length: 100%),
    )
  }
  ...
  if footer != none {
    ...
    footer = stack(
      spacing: base,
      line(length: 100%),
      text(size: base / 0.6, top-edge: base, footer),
    )
  }

  // TODO:
  // make margins and headers ignore 1st page
  set page(
    margin: margin,
    header: context {
  ...
  show link: if link_underline { underline } else { it => it }
